...
const DEFAULT_COURSES = [
  { name: 'PEED',   emoji: '⚡', color: '#7c6af7', status: 'active', priority: 'high' },
  ...
  { name: 'MD',     emoji: '📐', color: '#f43f5e', status: 'active', priority: 'medium' },
...
const STATUS_COLOR = { active: '#10b981', backlog: '#f59e0b', completed: '#6b6b78' }
const PRIORITY_COLOR = { high: '#f43f5e', medium: '#f59e0b', low: '#6b6b78' }
const PRIORITY_ORDER = { high: 0, medium: 1, low: 2 }

const EMPTY_FORM = { name: '', emoji: '📚', color: '#7c6af7', status: 'active', priority: 'medium' }
  ...
  return (
    <div className="px-4 pt-8 pb-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold" style={{ color: '#e8e8ec' }}>Courses</h1>
        <button
          onClick={openAdd}
      ...
      {loading ? (
        ...
        <p className="text-center py-16 text-sm" style={{ color: '#6b6b78' }}>No courses here</p>
      ) : (
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #2a2a30', backgroundColor: '#111113' }}>
      ...
      <div className="p-3 space-y-2.5">
        ...
        <p className="font-bold text-sm leading-tight" style={{ color: '#e8e8ec' }}>{course.name}</p>
      ...
      >
        ...
        <div className="flex gap-2">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: '#6b6b78' }}>Emoji</label>
            <input
              type="text"
              value={form.emoji}
          ...
          <div className="flex-1 flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: '#6b6b78' }}>Name</label>
            <input
              type="text"
              value={form.name}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium" style={{ color: '#6b6b78' }}>Color</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: '#6b6b78' }}>Status</label>
            <select
              value={form.status}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium" style={{ color: '#6b6b78' }}>Priority</label>
            <select
              value={form.priority}
        ...
        {/* Live preview */}
        <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #2a2a30' }}>
          <div className="h-1" style={{ backgroundColor: form.color }} />
          <div className="px-3 py-2.5 flex items-center gap-3" style={{ backgroundColor: '#1a1a1e' }}>
            <span className="text-2xl leading-none">{form.emoji}</span>
            <div>
              <p className="text-sm font-bold" style={{ color: '#e8e8ec' }}>{form.name || 'Course name'}</p>
              <p className="text-xs capitalize" style={{ color: '#6b6b78' }}>{form.status} · {form.priority}</p>
            </div>
          </div>
        </div>
    ...
    >
      ...
      >
        <div className="text-center space-y-1.5">
          ...
          <p className="font-bold text-base" style={{ color: '#e8e8ec' }}>Delete {course.name}?</p>
